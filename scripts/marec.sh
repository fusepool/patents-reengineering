...
find "$marec" -name *.xml | while read i ; do path=${i%/*} ; file=$(basename $i) ; fn=${file%.*}; saxonb-xslt -ext:on -tree:linked -s "$i" -xsl ../src/main/resources/xsl/marec.xsl pathToCPCConcordances="$pathToCPCConcordances" > "$marecrdf""$fn".rdf ; done
